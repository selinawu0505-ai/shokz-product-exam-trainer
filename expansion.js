...
CULTURE_QUESTIONS.forEach(q=>addExpanded({
  cat:'企业文化',
  cultureArea:CULTURE_AREAS[q.culture].name,
  ...q
}));
...
function mixedDaily(){
  ...
  let used=new Set(chosen.map(q=>q.id));
  const cultureNeed=Math.max(0,3-chosen.filter(q=>q.cat==='企业文化').length);
  chosen.push(...poolUnseen(QUESTIONS.filter(q=>q.cat==='企业文化'&&!used.has(q.id)),cultureNeed,'daily-culture'));
  used=new Set(chosen.map(q=>q.id));
  chosen.push(...poolUnseen(QUESTIONS.filter(q=>!used.has(q.id)),Math.max(0,15-chosen.length),'daily-extra'));
  return shuffle(chosen);
}
function examTypeSet(type,total,cultureCount){
  const culture=poolUnseen(QUESTIONS.filter(q=>q.type===type&&q.cat==='企业文化'),cultureCount,`exam-culture-${type}`);
  const used=new Set(culture.map(q=>q.id));
  const rest=poolUnseen(QUESTIONS.filter(q=>q.type===type&&q.cat!=='企业文化'&&!used.has(q.id)),total-culture.length,`exam-${type}`);
  return [...culture,...rest];
}
function examSet(){
  return shuffle([
    ...examTypeSet('single',10,3),
    ...examTypeSet('multi',5,2),
    ...examTypeSet('judge',5,2),
    ...examTypeSet('fill',10,3),
    ...examTypeSet('spec',1,0),
    ...examTypeSet('short',1,0)
  ]);
}

function home(){ clearInterval(timerId); session=null; const p=pct(),wrongCount=Object.keys(data.wrong).length,cultureCount=QUESTIONS.filter(q=>q.cat==='企业文化').length; app(`<main class="shell">${topbar()}
  <section class="hero"><div class="eyebrow">${QUESTIONS.length} 道动态题库 · 六种真题题型</div><h1>三条闯关路线，<br>产品文化一起拿下。</h1><p>今日闯关优先抽取当天未做过的题，并保证企业文化稳定出现。你可以按题型补短板、锁定某款产品，或集中攻克品牌文化。</p><div class="hero-actions"><button class="primary" onclick="startPractice()">今日混合闯关</button><button class="secondary" onclick="zone('type')">按题型闯关</button><button class="secondary" onclick="zone('product')">按产品闯关</button><button class="secondary" onclick="cultureZone()">企业文化专区</button><button class="ghost" onclick="guide()">冲刺手册</button></div><div class="goal-ring" style="--score:${Math.min(p,100)}"><span>${p}<small>当前正确率</small></span></div></section>
  <div class="grid"><div class="card stat"><div class="label">题库总量</div><div class="value">${QUESTIONS.length}</div><div class="muted">产品参数 + 企业文化</div></div><div class="card stat"><div class="label">文化题库</div><div class="value">${cultureCount}</div><div class="muted">覆盖2026品牌手册</div></div><div class="card stat"><div class="label">错题库存</div><div class="value">${wrongCount}</div><div class="muted">答对后自动移除</div></div><div class="card stat"><div class="label">模拟最高分</div><div class="value">${data.best}</div><div class="muted">目标至少 88 分</div></div></div>
  <div class="section-title"><h2>三个专项专区</h2><p>题型、产品、企业文化分别突破</p></div><div class="grid"><article class="card zone-pair" onclick="zone('type')"><div class="zone-index">A</div><h3>按照题型闯关</h3><p>完整覆盖真实试卷的单选、多选、判断、填空、综合填空和简答题。</p><div>${Object.values(TYPE_META).map(x=>`<span class="type-pill">${x.name}</span>`).join('')}</div></article><article class="card zone-pair" onclick="zone('product')"><div class="zone-index">B</div><h3>按照产品闯关</h3><p>七款产品独立题库，围绕参数、技术、定位、对比和销售话术集中训练。</p><div>${PRODUCT_SPECS.slice(0,4).map(x=>`<span class="type-pill">${x.name}</span>`).join('')}<span class="type-pill">+3</span></div></article><article class="card zone-pair culture-card" onclick="cultureZone()"><div class="zone-index">C</div><h3>企业文化专区</h3><p>${cultureCount} 道题，覆盖品牌理念、发展历程、全球化、科技创新、体育社群与 ESG。</p><div>${Object.values(CULTURE_AREAS).slice(0,5).map(x=>`<span class="type-pill">${x.name}</span>`).join('')}<span class="type-pill">+1</span></div></article></div>
...
function cultureZone(){
  const items=Object.entries(CULTURE_AREAS);
  app(`<main class="shell">${topbar(true)}<section class="zone-banner culture-banner"><div><div class="eyebrow">CULTURE ZONE</div><h1>企业文化闯关</h1><p>依据《2026品牌介绍》逐页梳理，按六大主题主动回忆。</p></div><div class="hero-actions"><button class="secondary" onclick="cultureGuide()">先看文化速记</button><button class="ghost" onclick="home()">切换专区</button></div></section><div class="grid" style="margin-top:18px">${items.map(([key,m])=>{const pool=QUESTIONS.filter(q=>q.culture===key);return `<article class="card deck" onclick="startByCulture('${key}')"><div class="count">${pool.length} 道题</div><h3>${m.name}</h3><p>${m.desc}</p><div class="go">开始文化闯关 →</div></article>`}).join('')}</div></main>`);
}
function cultureGuide(){
  const timeline=[['2011','AfterShokz 品牌成立'],['2013','启动中国市场布局'],['2015','技术／产品里程碑'],['2018','Shokz Team 启动'],['2019','顶级运动赛事 IP 合作'],['2021','基普乔格成为大中华区品牌代言人'],['2022','Shokz 品牌焕新，DTC 战略启动'],['2023','全球首届韶音运动嘉年华落地'],['2024','李现成为全球品牌代言人'],['2025','品牌战略升级，迈入开放聆听新蓝态'],['2026','重塑开放声学新标杆，引领行业新纪元']];
  app(`<main class="shell">${topbar(true)}<section class="guide-hero"><div class="eyebrow">CULTURE NOTES</div><h1>企业文化速记</h1><p class="muted">先背原句，再记年份与数字，最后补体育、社群和 ESG。</p><div class="hero-actions"><button class="primary" onclick="startByCulture('belief')">从品牌理念开练</button><button class="secondary" onclick="cultureZone()">返回文化专区</button></div><nav class="guide-nav"><a href="#culture-core">核心原句</a><a href="#culture-beopen">BE OPEN</a><a href="#culture-time">发展时间线</a><a href="#culture-data">关键数字</a><a href="#culture-esg">体育与ESG</a></nav></section>
  <section class="card guide-section" id="culture-core"><h2>必须逐字记忆</h2><div class="guide-columns"><div class="guide-mini"><h3>品牌主张</h3><p>开放聆听，自在相联。</p></div><div class="guide-mini"><h3>文化解释</h3><p>尽善尽美，科技向善。</p></div><div class="guide-mini"><h3>名称来源</h3><p>“子在齐闻《韶》，三月不知肉味。”出自《论语·述而》；《韶》相传为舜帝时期宫廷乐舞。</p></div><div class="guide-mini"><h3>使命式表达</h3><p>用科技创新推动世界进步，让人类生活更美好。</p></div></div></section>
  <section class="card guide-section" id="culture-beopen"><h2>BE OPEN 三层六句</h2><div class="guide-table-wrap"><table class="guide-table"><thead><tr><th>层级</th><th>完整表达</th></tr></thead><tbody><tr><td>物理层</td><td>开放聆听，享听觉自由</td></tr><tr><td>体验层</td><td>场景无界，专注亦在场</td></tr><tr><td>精神层</td><td>心无壁垒，开放即联结</td></tr></tbody></table></div><div class="guide-callout">易错：体验层与精神层最容易互换；“尽善尽美，科技向善”不属于 BE OPEN 三层原句。</div></section>
  <section class="card guide-section" id="culture-time"><h2>2011—2026 发展时间线</h2><div class="guide-table-wrap"><table class="guide-table"><thead><tr><th>年份</th><th>品牌事件</th></tr></thead><tbody>${timeline.map(x=>`<tr><td>${x[0]}</td><td>${x[1]}</td></tr>`).join('')}</tbody></table></div></section>
  <section class="card guide-section" id="culture-data"><h2>五组必考数字</h2><div class="guide-columns"><div class="guide-mini"><h3>60余</h3><p>远销全球60余个国家及地区</p></div><div class="guide-mini"><h3>8</h3><p>全球8大办公地点</p></div><div class="guide-mini"><h3>3000+</h3><p>全球团队成员</p></div><div class="guide-mini"><h3>2557 / 8200余</h3><p>2557项开放式耳机发明专利申请；8200余项知识产权全球布局</p></div></div><h3>SHOKZ LAB</h3><p>骨传导声学、开放式声学、运动科技。</p><div class="guide-callout">数字陷阱：2557和8200余绝不能对调。</div></section>
  <section class="card guide-section" id="culture-esg"><h2>体育、社群与 ESG</h2><div class="guide-columns"><div class="guide-mini"><h3>GLOCAL</h3><p>GLOBAL：全球化品牌内核；LOCAL：本地化价值交付。</p></div><div class="guide-mini"><h3>科技向善</h3><p>无痕山野、绿色可持续、融合社区。</p></div><div class="guide-mini"><h3>运动员矩阵</h3><p>基普乔格；马拉松康纳·曼茨；越野跑申加升；铁人三项扬·弗勒德诺。</p></div><div class="guide-mini"><h3>四大社群</h3><p>韶音开跑吧、韶音骑行家、韶音能量场、韶音运动趴。</p></div></div></section></main>`);
}
...
function startByCulture(culture){ const pool=QUESTIONS.filter(q=>q.culture===culture); begin(poolUnseen(pool,Math.min(15,pool.length),`culture-${culture}`),'practice',CULTURE_AREAS[culture].name+'专项'); }
